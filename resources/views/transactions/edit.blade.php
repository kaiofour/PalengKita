                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('transactions.update', $transaction->transaction_id) }}">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-4">
                                <input id="overall_price" type="number" step="0.01" class="form-control @error('overall_price') is-invalid @enderror" name="overall_price" value="{{ old('overall_price', $transaction->overall_price) }}" required>
                                @error('overall_price')
                                    <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                                @enderror
                            </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-8" id="cart-items">
                                @php
                                    $cart = is_string($transaction->cart) ? json_decode($transaction->cart, true) : $transaction->cart;
                                @endphp

                                @foreach($cart as $index => $item)
                                    <div class="row mb-2 cart-item">
                                        <div class="col-md-6">
                                            <input type="number" name="cart[{{ $index }}][product_id]" placeholder="Product ID" class="form-control" value="{{ $item['product_id'] }}" required>
                                        </div>
                                        <div class="col-md-4">
                                            <input type="number" name="cart[{{ $index }}][qty]" placeholder="Quantity" class="form-control" value="{{ $item['qty'] }}" required>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
